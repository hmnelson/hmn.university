...
class HiddenProfiles(object):
    def getNonInstallableProfiles(self):
        ...
        return [
            "hmn.university:uninstall",
        ]

    def getNonInstallableProducts(self):
        """Hide the upgrades package from site-creation and quickinstaller."""
        return ["hmn.university.upgrades"]
...
def add_catalog_indexes(context):
    ...
    missing_zctextindex = [w for w in wanted_zctextindex if w not in indexes]
    if missing_zctextindex:
        for name in missing_zctextindex:
            item_extras = Empty()
            item_extras.doc_attr = name
            item_extras.index_type = 'Okapi BM25 Rank'
            item_extras.lexicon_id = 'plone_lexicon'
            catalog.addIndex(name, 'ZCTextIndex', item_extras)
            logger.info("Added ZCTextIndex for field %s.", name)
        logger.info("Indexing new field indexes %s.", ', '.join(missing_zctextindex))
        catalog.manage_reindexIndex(ids=missing_zctextindex)
