    ...
    def __call__(self):
        ...
        if len(departments) > 0:
            ...
            if 'Dept Head' in local_roles:
                ...
                return self.context.REQUEST.RESPONSE.redirect(self.context.portal_url() + department.getPath() + '/teacher_list')
            if 'Teacher' in local_roles:
                return self.context.REQUEST.RESPONSE.redirect(self.context.portal_url() + department.getPath() + '/')
            # return self.context.REQUEST.RESPONSE.redirect(self.context.portal_url() + '/folder_contents')
